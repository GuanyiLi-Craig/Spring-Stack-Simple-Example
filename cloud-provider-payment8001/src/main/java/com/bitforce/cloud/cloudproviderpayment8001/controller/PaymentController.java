package com.bitforce.cloud.cloudproviderpayment8001.controller;
...
import com.bitforce.cloud.cloudproviderpayment8001.service.PaymentService;
...
public class PaymentController {
    ...
    @GetMapping(value="/payment/send/{id}")
    public CommonResult<String> sendPaymentById(@PathVariable("id") Long id) {
        Payment payment = paymentService.getPaymentById(id);
        log.info("Create Result: " + payment);

        if (payment != null) {
            String sendResult = messageProviderService.send(id);
            return new CommonResult<>(200, "Send payment id succesfully server " + serverPort, sendResult);
        } else {
            return new CommonResult<>(444, "Send payment failed where id is " + id + " server " + serverPort, null);
        }
    }
